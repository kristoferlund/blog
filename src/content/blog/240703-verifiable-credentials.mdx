...
The main use case for this project is to allow the user to perform an action in the demo app that requires a certain Gitcoin Passport score level. Before we can perform the action, we need to make sure the issuer knows the user's Gitcoin Passport score and is able to issue a Verifiable Credential to prove it.
